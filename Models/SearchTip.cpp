...
void SearchTip::insert(const string &key) {
    Ref<Query> q = query();
    RefArray arr = q->equal("key", key)->results();
    Ref<SearchTip> tip;
    if (arr.size() == 0) {
        tip = new SearchTip;
        tip->setKey(key);
...
RefArray SearchTip::search(const string &key) {
    string search_key = "%";
    if (!key.empty()) {
        search_key += key + '%';
    }
    RefArray rets;
    Ref<Query> q = query();
    RefArray arr = q->like("key", search_key)->sortBy("date")->limit(5)->results();
    for (auto it = arr->begin(), _e = arr->end(); it != _e; ++it) {
        Ref<SearchTip> t = *it;
        rets.push_back(t->getKey());
    }
    return rets;
}
